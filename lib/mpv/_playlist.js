...
const playlist = {
					...
					// continue the skipping process
						...
						.then((nextTrackFile) => {
							...
							observeSocket.connect({path: this.options.socket}, () =>{
								...
								this.command('playlist-next', [mode])
								.then(() => {
									// timeout
									let timeout = 0;
									// check if the file was started
	...
	prev: function(mode = 'weak') {
		return new Promise((resolve, reject) => {
			...
			if(!["weak", "force"].includes(mode)){
				...
				return this.getPlaylistPosition1()
				.then((position) => {
					...
					// continue the skipping process
						...
						.then((prevTrackFile) => { return new Promise((resolve, reject) => {
							...
							observeSocket.connect({path: this.options.socket}, () =>{
								...
								this.command('playlist-prev', [mode])
								.then(() => {
									// timeout
									let timeout = 0;
									// check if the file was started
	...
	goto: function(newPosition) {
		return new Promise((resolve, reject) => {
			...
			if (!this.running) {
				return reject(
					this.errorHandler.errorMessage(8, null, 'goto()')
				);
			}
			if (this.observed["playlist-pos"] === newPosition){
				this.goToPosition(0).then(() => {
					return resolve(true);
				});
			} else {
				return this.getProperty(`playlist`)
				.then((playlist) => {
					if (newPosition >= 0 && newPosition < playlist.length) {
						// socket to observe the command
						const observeSocket = net.Socket();
						observeSocket.connect({path: this.options.socket}, () =>{
							// send the prev command to mpv
							this.setProperty('playlist-pos', newPosition)
							.then(() => {
								// timeout
								let timeout = 0;
								// check if the file was started
								...
								observeSocket.on('data', (data) => {
										...
										if(message.length > 0){
											...
											if('event' in message){
												...
												// reject the promise if the file could not be loaded
													...
													observeSocket.destroy();
													return reject(this.errorHandler.errorMessage(0, newPosition, 'goto()'));
												}
											}
										}
									});
									...
									if(timeout > 10){
										observeSocket.destroy();
										return reject(this.errorHandler.errorMessage(5, null, 'goto()'));
									}
								});
							});
						});
						...
						reject(
							this.errorHandler.errorMessage(1, null, 'goto()')
						);
					}
				});
			}
