...
const playlist = {
	...
	loadPlaylist: function(playlist, mode = 'replace'){
		return new Promise((resolve, reject) => {
			...
			if(!['replace', 'append'].includes(mode)){
				return reject(this.errorHandler.errorMessage(1, mode, 'loadPlaylist()', {
					'replace': 'replace the current playlist (default)',
					'append':  'append to the current playlist'
				}));
			}

			// check of the playlistfile exists
			stat(playlist, (err, stats) => {
				if(err && err.errno == -2){
					return reject(this.errorHandler.errorMessage(0, playlist, 'loadPlaylist()'));
				}
				else{
					return resolve();
				}
			...
			observeSocket.connect({path: this.options.socket}, () =>{
				...
				observeSocket.on('data', (data) => {
						...
						if(message.length > 0){
							...
							if('event' in message){
								...
								// when the track has changed we don't need a seek event
									...
									.then((filename) => {
										return reject(this.errorHandler.errorMessage(0, filename, 'loadPlaylist()'));
									})
								}
							}
						}
					...
					if(timeout > 10){
						...
						reject(this.errorHandler.errorMessage(5, null, 'loadPlaylist()'));
					}
				});
			});
		});
	...
	next: function(mode = 'weak') {
		...
		.then((nextTrackFile) => { return new Promise((resolve, reject) => {
			...
			if(!["weak", "force"].includes(mode)){
				reject(this.errorHandler.errorMessage(1, mode, 'next()', {
			...
			observeSocket.connect({path: this.options.socket}, () =>{
				...
				observeSocket.on('data', (data) => {
						...
						if(message.length > 0){
							...
							if('event' in message){
								...
								// reject the promise if the file could not be loaded
									...
									reject(this.errorHandler.errorMessage(0, nextTrackFile, 'next()'));
								}
					...
					// to prevent having sockets listening forever
					if(timeout > 10){
						reject(this.errorHandler.errorMessage(5, null, 'next()'));
					}
				});
			});
		});
	...
	prev: function(mode = 'weak') {
		...
		.then((prevTrackFile) => { return new Promise((resolve, reject) => {
			...
			// reject the promise if the mode is not correct
			if(!["weak", "force"].includes(mode)){
				reject(this.errorHandler.errorMessage(1, mode, 'prev()', {
