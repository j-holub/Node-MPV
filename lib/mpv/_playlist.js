		...
		return new Promise((resolve, reject) => {
			...
			.then((size) => {
				...
				if (position >= 0 && position < size) {
					...
					// not be loaded
					this.getProperty(`playlist/${position}/filename`)
					.then((filename) => {
						// socket to observe the command
						const observeSocket = net.Socket();
						observeSocket.connect({path: this.options.socket}, () =>{
							// send the command to jump in the playlist to mpv
							this.setProperty('playlist-pos', position)
							.then(() => {
								// timeout
								let timeout = 0;
								// check if the file was started
								...
								observeSocket.on('data', (data) => {
										...
										if(message.length > 0){
											...
											if('event' in message){
												...
												// reject the promise if the file could not be loaded
													...
													observeSocket.destroy();
													return reject(this.errorHandler.errorMessage(0, 'jump()', filename));
												}
											}
										}
									});
									...
									if(timeout > 10){
										observeSocket.destroy();
										return reject(this.errorHandler.errorMessage(5, null, 'jump()'));
									}
								});
							});
						});
