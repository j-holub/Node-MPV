...
const playlist = {
	...
	next: async function(mode = 'weak') {
		...
		const nextTrackFile = await this.getProperty(`playlist/${this.observed['playlist-pos']+1}/filename`);
	...
	prev: async function(mode = 'weak') {
		...
		const prevTrackFile = await this.getProperty(`playlist/${this.observed['playlist-pos']-1}/filename`);
		return new Promise((resolve, reject) => {
			// socket to observe the command
			const observeSocket = net.Socket();
			observeSocket.connect({path: this.options.socket}, async () =>{
