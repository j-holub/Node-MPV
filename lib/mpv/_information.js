...
const information = {
	...
	getArtist: function() {
		...
		.then((metadata) => metadata && metadata.artist);
	},
	...
	getAlbum: function() {
		return this.getMetadata()
		.then((metadata) => metadata && metadata.album);
	},
	...
	getYear: function() {
		return this.getMetadata()
		.then((metadata) => metadata && metadata.date);
	},
	...
	getFilename: function(format = 'full') {
		...
		if(!['stripped', 'full'].includes(format)){
			return Promise.reject(this.errorHandler.errorMessage(1, 'getFilename()', [format], null, {
