...
const startStop = {
	...
	start: function() {
		...
		.then((ipcCommand) => {
			return new Promise((resolve, reject) => {
				// If the option already_running is set to true, we'll assume mpv is
				// already running and the ipc server is listening.
				// This is the user's responsibility, I'm not sure how we can test if the
				// IPC server is really running.
				if(this.options.already_running) resolve();
