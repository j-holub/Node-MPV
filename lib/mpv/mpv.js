	...
	load: function(file, mode = 'replace', options) {
		return new Promise((resolve, reject) => {
			...
			if(!['replace', 'append', 'append-play'].includes(mode)){
				return reject(`'${mode}' is not a valid mode. Must be one of: \n replace \n append \n append-play`);
			}
			...
			observeSocket.connect({path: this.options.socket}, () =>{
				...
				this.socket.command('loadfile', options ? [file, mode].concat(util.formatOptions(options)) : [file, mode]);
				...
				observeSocket.on('data', (data) => {
						...
						if(message.length > 0){
							...
							if('event' in message){
								...
								// when the track has changed we don't need a seek event
									...
									observeSocket.destroy();
									reject(this.errorHandler.errorMessage(0, file, 'load()'));
								}
							}
						}
					});
					...
					if(timeout > 10){
						observeSocket.destroy();
						reject(this.errorHandler.errorMessage(5, null, 'load()'));
					}
				});
			});
		});
