...
mpv.prototype = Object.assign({
	...
	// options
	// further options
	load: async function(file, mode = 'replace', options) {
		...
		if (!this.running){
			throw (
				this.errorHandler.errorMessage(8, caller, options ? [file, mode].concat(options) : [file, mode], null, {
		...
		await new Promise ((resolve, reject) => {
			...
			observeSocket.connect({path: this.options.socket}, async () =>{
				...
				await this.command('loadfile', options ? [file, mode].concat(util.formatOptions(options)) : [file, mode]);
						...
						if(message.length > 0){
							...
							if('event' in message){
								...
								// when the track has changed we don't need a seek event
									...
									observeSocket.destroy();
									reject(this.errorHandler.errorMessage(0, caller, [file]));
								}
							}
						}
					});
