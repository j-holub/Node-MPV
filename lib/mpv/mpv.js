...
function mpv(options, mpv_args){
	...
	var defaultArgs = [ipcCommand + "=" + this.options.socket, '--idle', '--really-quiet', '--msg-level=ipc=v'];
	...
	this.mpvPlayer = spawn((options.binary ? options.binary : 'mpv'), defaultArgs);
	...
	// reject
		...
		console.log(this.options);
		if(this.options.verbose){
			console.log("Could not bind IPC socket. Quitting...");
		}
	}.bind(this));
