...
const ipcInterface = function(options) {
	...
	// dictionary to store the promise resolve and reject functions
    // of the messages sent over the IPC socket
	this.messageResolve = {}
	this.messageReject  = {}
...
ipcInterface.prototype = Object.assign({
	...
	send: function(command) {
		return new Promise((resolve, reject) => {
			...
			this.messageResolve[request_id] = resolve;
			this.messageReject[request_id]  = reject;
			try{
				this.socket.write(JSON.stringify(messageJson) + '\n');
			}
			// reject the promise in case of an error
			catch(error) {
				return reject(this.errorHandler.errorMessage(7, message, 'send()'));
			}
		})
