				...
				if('request_id' in JSONmessage){
					// resolve promise
				 	if(JSONmessage.error === 'success'){
						// resolve the request's stored resolve function
						this.messageResolve[JSONmessage.request_id](JSONmessage.data);
						// delete the functions from the maps
						delete this.messageResolve[JSONmessage.request_id];
						delete this.messageReject[JSONmessage.request_id];
					}
					// reject promise
					else{
						// reject the message's promise
						this.messageReject[JSONmessage.request_id](this.errorHandler.errorMessage(3, JSONmessage.error, 'send()'));
						// delete the functions from the maps
						delete this.messageResolve[JSONmessage.request_id];
						delete this.messageReject[JSONmessage.request_id];
					}
